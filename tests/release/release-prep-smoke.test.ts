...
describe("release prep smoke", () => {
  ...
  it("keeps the beta release candidate blocked until evidence and owners are ready", () => {
    ...
    expect(sprint46Story).toMatch(/Stabilization process is documented/i);
    expect(sprint46Story).toMatch(/real NFS-e issuance, scraping, municipal provider, certificate and fiscal job paths blocked/i);
    expect(stabilizationPlan).toMatch(/P0 \| Tenant leak, secret leak, real fiscal action/i);
    expect(stabilizationPlan).toMatch(/Post-Pilot Release Candidate Criteria/i);
    expect(stabilizationPlan).toMatch(/No real NFS-e issuance/i);
    expect(sprint47Story).toMatch(/NO implementation of real NFS-e issuance starts in this sprint/i);
    expect(realNfsePrd).toMatch(/This PRD does not authorize implementation/i);
    expect(realNfsePrd).toMatch(/No certificate upload or storage/i);
    expect(realNfsePrd).toMatch(/Provider timeout must not automatically create a second fiscal transmission/i);
    expect(homologationAdr).toMatch(/This ADR does not authorize implementation/i);
    expect(homologationAdr).toMatch(/No real issuance without explicit PRD, provider ADR, homologation evidence and go\/no-go approval/i);
