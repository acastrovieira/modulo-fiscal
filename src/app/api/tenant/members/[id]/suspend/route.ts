...
import { suspendTenantMemberRequestSchema } from "@/modules/tenant/presentation/tenant-admin-schemas";
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    suspendTenantMemberRequestSchema.parse(await request.json().catch(() => ({})));
    ...
    const data = await service.suspendMember({ context: commandContext, membershipId: id });
