...
export async function POST(request: Request) { const requestId = createCorrelationId(); try { const context = await createCommandContext({ correlationId: requestId }); const body = createBatchRequestSchema.parse(await request.json()); const repository = createPrismaFiscalBatchRepository(); const service = createFiscalBatchService({ repository, audit }); const batch = await service.createFiscalBatch({ context, candidateIds: body.candidateIds, batchNumber: body.batchNumber }); const data = toBatchDTO({ ...batch, createdAt: new Date(), updatedAt: new Date(), itemsCount: batch.items?.length ?? 0 }); return NextResponse.json({ data, requestId }, { status: 201 }); } catch (error) { return apiErrorResponse(error, requestId); } }